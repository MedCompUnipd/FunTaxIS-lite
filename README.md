...
- `config_file.cfg`, the configuration file that contains the following parameters:
    ...
    - `species`: path of the file containing the list of species of interest. The user can modify this list at will by putting the taxonomic ID from the NCBI taxonomy database of the species and/or branches of the taxonomy tree for which constraints are to be produced. Without this parameter the script uses the species.txt file in add_files folder. (Optional. Default: use the species file in add_files directory.)
